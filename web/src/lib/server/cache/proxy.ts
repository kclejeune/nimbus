...
export function touchFailed(cacheId: number, hash: string): void {
	// Retry promptly after failure, without hammering an unavailable backend.
	recentTouches.set(`${cacheId}:${hash}`, true, 5_000);
}
...
const proxyKeypair = new AsyncMemo<string>(Number.MAX_SAFE_INTEGER, 1);

export function getProxyKeypair(env: Env): Promise<string> {
	return proxyKeypair.get('key', () => loadProxyKeypair(env));
}
